...
public class f implements Interpolator {
    ...
    private void b(Path path) {
        ...
        if (min > 0) {
            this.f622a = new float[min];
            this.f623b = new float[min];
            float[] fArr = new float[2];
            for (int i2 = 0; i2 < min; i2++) {
                pathMeasure.getPosTan((((float) i2) * length) / ((float) (min - 1)), fArr, (float[]) null);
                this.f622a[i2] = fArr[0];
                this.f623b[i2] = fArr[1];
            }
            if (((double) Math.abs(this.f622a[0])) <= 1.0E-5d && ((double) Math.abs(this.f623b[0])) <= 1.0E-5d) {
                int i3 = min - 1;
                if (((double) Math.abs(this.f622a[i3] - 1.0f)) <= 1.0E-5d && ((double) Math.abs(this.f623b[i3] - 1.0f)) <= 1.0E-5d) {
                    float f2 = 0.0f;
                    int i4 = 0;
                    while (i < min) {
            ...
            sb.append(this.f622a[0]);
            sb.append(",");
            sb.append(this.f623b[0]);
            sb.append(" end:");
            int i6 = min - 1;
            sb.append(this.f622a[i6]);
            sb.append(",");
            sb.append(this.f623b[i6]);
    ...
    public float getInterpolation(float f2) {
        ...
        int i = 0;
        int length = this.f622a.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f2 < this.f622a[i2]) {
                length = i2;
        ...
        float[] fArr = this.f622a;
        float f3 = fArr[length] - fArr[i];
        if (f3 == 0.0f) {
            return this.f623b[i];
        }
        float[] fArr2 = this.f623b;
        float f4 = fArr2[i];
