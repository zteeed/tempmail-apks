...
public class TextLayer extends BaseLayer {
    private final d<String> codePointCache = new d<>();
    ...
    private String codePointToString(String str, int i) {
        ...
        if (this.codePointCache.e(j)) {
            return this.codePointCache.i(j);
        }
    ...
    private void drawCharacterAsGlyph(FontCharacter fontCharacter, Matrix matrix2, float f2, DocumentData documentData, Canvas canvas) {
    ...
    private void drawGlyphTextLine(String str, DocumentData documentData, Matrix matrix2, Font font, Canvas canvas, float f2, float f3) {
        float floatValue;
        for (int i = 0; i < str.length(); i++) {
            FontCharacter f4 = this.composition.getCharacters().f(FontCharacter.hashFor(str.charAt(i), font.getFamily(), font.getStyle()));
            if (f4 != null) {
                drawCharacterAsGlyph(f4, matrix2, f3, documentData, canvas);
                float width = ((float) f4.getWidth()) * f3 * Utils.dpScale() * f2;
                ...
                if (baseKeyframeAnimation != null) {
                    ...
                    canvas.translate(width + (f5 * f2), 0.0f);
                }
                f5 += floatValue;
                canvas.translate(width + (f5 * f2), 0.0f);
            }
    ...
    private void drawTextGlyphs(DocumentData documentData, Matrix matrix2, Font font, Canvas canvas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            drawGlyphTextLine(str2, documentData, matrix2, font, canvas, scale, f3);
            canvas.restore();
        }
    }
    ...
    private float getTextLineWidthForGlyphs(String str, Font font, float f2, float f3) {
        float f4 = 0.0f;
        for (int i = 0; i < str.length(); i++) {
            FontCharacter f5 = this.composition.getCharacters().f(FontCharacter.hashFor(str.charAt(i), font.getFamily(), font.getStyle()));
            if (f5 != null) {
                f4 = (float) (((double) f4) + (f5.getWidth() * ((double) f2) * ((double) Utils.dpScale()) * ((double) f3)));
            }
        }
        return f4;
    }
