...
class BinaryImagesConverter {
    ...
    private File correctDataPath(File file) {
        ...
        try {
            ...
            Logger.getLogger().e("Error getting ApplicationInfo", e2);
            return file;
        }
    }
    ...
    private static JSONObject createBinaryImageJson(String str, ProcMapEntry procMapEntry) throws JSONException {
        ...
        jSONObject.put(AnalyticsConnectorReceiver.EVENT_NAME_KEY, procMapEntry.path);
    ...
    private static byte[] generateBinaryImagesJsonString(JSONArray jSONArray) {
        ...
        try {
            ...
            Logger.getLogger().w("Binary images string is null", e2);
            return new byte[0];
        }
    }
        ...
        if (parse != null && isRelevant(parse)) {
            try {
                try {
                    ...
                    Logger.getLogger().d("Could not create a binary image json string", e2);
                    return null;
                }
            } catch (IOException e3) {
                Logger logger = Logger.getLogger();
                logger.d("Could not generate ID for file " + parse.path, e3);
            }
        }
        return null;
    }
    ...
    private JSONArray parseProcMapsJsonFromString(String str) {
        ...
        try {
            ...
            Logger.getLogger().w("Unable to parse proc maps string", e2);
            return jSONArray;
        }
    }
